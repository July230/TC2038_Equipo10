...
int main(){
    ...
    transmisiones.push_back("ncsacnsajncsacreconocernjdskavjdskabvsblrervndjsavdjsakdvdasvsajlk12321vdavanvlkjavasdjvdab");

    cout << preProcesar(cadena) << endl;

    // Recorrer las transmisiones y utilizar el algoritmo de manacher
    for(int i = 0; i < transmisiones.size(); i++){
        pair<int, int> res = manacher(transmisiones[i]);
        cout << "Posiciones del palindromo mas largo " << res.first << " " << res.second + 1 << endl;

        // Imprimir el palindromo mas largo utilizando las posiciones 1-based
        cout << "El palíndromo más largo es: ";
        cout << transmisiones[i].substr(res.first, res.second - res.first + 1) << endl;
    }
